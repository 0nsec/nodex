...
import org.nodex.api.blog.BlogConstants;
...
class BlogPostFactoryImpl implements BlogPostFactory {
    ...
    public BlogPost createBlogPost(GroupId groupId, long timestamp,
        ...
        if (textLength > BlogConstants.MAX_BLOG_POST_TEXT_LENGTH)
        ...
        byte[] sig = clientHelper
                .sign(BlogPostFactory.SIGNING_LABEL_POST, signed, author.getPrivateKey());
    ...
    public Message createBlogComment(GroupId groupId, LocalAuthor author,
        ...
        if (comment != null) {
            ...
            if (commentLength > BlogConstants.MAX_BLOG_COMMENT_TEXT_LENGTH)
                throw new IllegalArgumentException();
        }
        ...
        byte[] sig = clientHelper
                .sign(BlogPostFactory.SIGNING_LABEL_COMMENT, signed, author.getPrivateKey());
